...
function renderTax() {
  const tpl = (values, empty) => values.length
    ? values.slice(0, 8).map(value => `<span class="tax-pill">${esc(value)}</span>`).join("")
    : `<span class="tax-pill" style="color:var(--muted2);">${esc(empty)}</span>`;

  document.getElementById("seriesTax").innerHTML = tpl(state.albumGroups.map(group => group.name), "Collections appear here");
  document.getElementById("locationTax").innerHTML = tpl(uniq(state.photos.map(photo => photo.location)), "Places appear here");
  document.getElementById("cameraTax").innerHTML = tpl(uniq(state.photos.map(photo => photo.camera)), "Cameras appear here");
}
